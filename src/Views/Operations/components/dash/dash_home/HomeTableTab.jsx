...
export default function HomeTableTab({datasummary}) {
  ...
  return (
    <div className='bg-whitex rounded mt-2 overflow-hidden shadow-lg'>
        <div className='ongiong-pending-tab h-14 p-2 xpb-5'>
            <span onClick={switch_to_OngoingProjects} className={OP}>Projects</span>
            <span onClick={switch_to_PendingBids} className={PB}>Bids</span>
        </div>
        <OngoingProjectsTr showr={showOngoingProjects} projects={datasummary?.projects}/>
        <PendingBidsTr showr={showPendingBids} bids={datasummary?.bids}/>
    </div>
  )
}
