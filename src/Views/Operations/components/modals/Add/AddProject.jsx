...
export default function AddProject(props) {
  const [createProjectHtmlValue, setCreateProjectHtmlValue] =useState("Create Project");
  const [createProjectIcon, setCreateProjectIcon] = useState(faPlus);
  const [createProjectIconClassname, setCreateProjectIconClassname] =useState();
  ...
  const handleBidSelection = async (e) => {
    ...
    if (data.status == 403) {
      ...
      toast.error(`Server error, could not get bid.Please check your network connection`, {
        icon: "😞",
        duration: 5000,
        position: "top-right",
        style: {
    ...
    <div className={props.show}>
      <div className="xback-board txt-dark-bluex">
        <div className="modal-inner-edit-x">
          ...
          <form
            className="grid grid-cols-4 grid-row-7 gap-4 px-5 py-6 " id="createProject-form"
