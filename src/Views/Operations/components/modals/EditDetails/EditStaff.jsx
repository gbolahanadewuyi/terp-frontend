...
export default function EditStaff(props) {
  //
  const [updateStaffHtmlValue, setUpdateStaffHtmlValue] = useState("Update");
  const prevUpdateStaffHtmlValue = usePrevious(updateStaffHtmlValue);

  const [updateStaffIcon, setUpdateStaffIcon] = useState(faPlus);
  const prevUpdateStaffIcon = usePrevious(updateStaffIcon);

  const [updateStaffIconClassname, setUpdateStaffIconClassname] = useState();
  const prevUpdateStaffIconClassname = usePrevious(updateStaffIconClassname);
  ...
  const handleSubmit = async (e) => {
    ...
    if (data.status == 403) {
      ...
      setUpdateStaffIconClassname(prevUpdateStaffIconClassname);
      setUpdateStaffIcon(prevUpdateStaffIcon);
      setUpdateStaffHtmlValue(prevUpdateStaffHtmlValue);
      props.handleShow();
    ...
    <div className={props.show}>
      <div className="xback-board txt-dark-bluex">
        <div className="modal-inner-edit-x2">
          ...
          <form className="grid grid-cols-3 gap-4 px-10 py-6" onSubmit={handleSubmit}>
