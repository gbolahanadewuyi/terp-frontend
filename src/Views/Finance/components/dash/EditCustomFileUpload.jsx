  ...
  function handleChange(event) {
    ...
    switch (props.input_id) {
      ...
      case "editinvoice":
        ...
        break;
      case "editcertificate-of-completion":
        setShowNotUploaded("hidden");
        setShowUploading("grid");
            ...
            case "editnsitf-file":
              ...
              case "editpencom-file":
                ...
                case "editsworn-affidavit-file":
                  ...
                  case "editbank-reference-letter-file":
                    setShowNotUploaded("hidden");
                    setShowUploading("grid");
                    ...
                    setShowNotUploaded("hidden");
                    setShowUploading("grid");
                    const apgInvoiceFile = event.target.files[0];
            
                    //create the file metadata
                    const apgInvoiceMetadata = {
                      contentType: apgInvoiceFile.type
                    };
            
                    //uploade file and metadata
                    const apgInvoiceStorageRef = ref(
                      storage,
                      "docs/" + apgInvoiceFile.name
                    );
                    const apgInvoiceUploadTask = uploadBytesResumable(
                      apgInvoiceStorageRef,
                      apgInvoiceFile,
                      apgInvoiceMetadata
                    );
            
                    //listen for state changes, errors and completiton of the upload
                    apgInvoiceUploadTask.on(
                      "state_changed",
                      (snapshot) => {
                        //get task progress, including the number of bytes uploaded and the total number of bytes to be uploaded
                        const progress =
                      ...
                      () => {
                        //Upload completed successfully, now we can get the download URL
                        getDownloadURL(apgInvoiceUploadTask.snapshot.ref).then(
                          (downloadURL) => {
                            console.log("File available at", downloadURL);
                            props.setApgInvoice(downloadURL);
                          }
                        );
                        setShowUploading("hidden");
                        setShowUploaded("grid");
