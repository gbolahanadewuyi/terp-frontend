...
async function updateTask(credentials, id) {
  ...
  try {
    const data = await fetch(
      `https://us-central1-terp-338409.cloudfunctions.net/app/api/updateTask?id=${id}`,
      {
        method: "POST",
        headers: {
...
export default function EditTask(props) {
  const [updateTaskHtmlValue, setUpdateTaskHtmlValue] = useState("Update Task");
  const prevUpdateTaskHtmlValue = usePrevious(updateTaskHtmlValue);

  const [updateTaskIcon, setUpdateTaskIcon] = useState(faPlus);
  const prevUpdateTaskIcon = usePrevious(updateTaskIcon);

  const [updateTaskIconClassname, setUpdateTaskIconClassname] = useState();
  const prevUpdateTaskIconClassname = usePrevious(updateTaskIconClassname);
  ...
  //list of staffs without the excluding the staff that was assigned
  const [assignedToStaffList, setAssignedToStaffList] = useState([]);
  //list of people involved that was previously selected
  const [prevSelectedPeopleInvolved, setPrevSelectedPeopleInvolved] = useState(
    []
  );
  ...
  useEffect(async () => {
    ...
    setScope(props.details?.scope);
    setAssignedTo(props.details?.assignedTo);
    setInitiatedBy(props.details?.initiatedBy);
    setDeadline(props.details?.deadline);
    setComment(props.details?.comment);
    setPriorityLevel(props.details?.prioritylevel);
    setPeopleInvolved(props.details?.peopleInvolved);
    setStatus(props.details?.status);

    //dropdown multi selected values
    setPrevSelectedPeopleInvolved(
      props.details?.peopleInvolved.map((el) => ({ name: el }))
    );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    document.getElementById("updateTask-button").disabled = false;
    setUpdateTaskHtmlValue("Processing please wait....");
    setUpdateTaskIcon(faSpinner);
    setUpdateTaskIconClassname("spinner");
    ...
    if (data.status == 403) {
      ...
      setUpdateTaskIconClassname("");
      setUpdateTaskIcon(faPlus);
      setUpdateTaskHtmlValue("Update Task");
      document.getElementById("updateTask-button").disabled = false;
    } else if (data.status == 200) {
      ...
      setUpdateTaskIcon(faPlus);
      setUpdateTaskHtmlValue("Update Task");
      document.getElementById("edittask-form").reset();
      document.getElementById("updateTask-button").disabled = false;
      props.handleShow();
  ...
  useEffect(async () => {
    setAssignedToStaffList(
      stafflist?.filter((e) => {
        return e.name !== assignedTo;
      })
    );
  }, [stafflist, assignedTo]);
      ...
      <div className="xback-board txt-dark-bluex">
        <div className="modal-inner-edit-x">
          <div className="modal-edit-head-v2 relative">
            ...
            <h1 className=" p-2 pl-5 edit-modal-headr-x">Edit Task</h1>
          </div>
          <form
            className="grid grid-cols-6 grid-row-7 gap-4 px-10 py-6"
            id="edittask-form"
          ...
          >
            ...
            <div className="col-span-3 text-left">
              ...
              <Multiselect
                ...
                selectedValues={prevSelectedPeopleInvolved}
            ...
            >
              ...
              <FontAwesomeIcon
                icon={updateTaskIcon}
                className={updateTaskIconClassname}
              />{" "}
              {updateTaskHtmlValue}{" "}
            </button>
          </form>
