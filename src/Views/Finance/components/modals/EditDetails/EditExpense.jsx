...
import expensesTemp from "../../../../../tempDb/expenses";
...
async function getExpense(id) {
  ...
  return fetch(
    `https://us-central1-terp-338409.cloudfunctions.net/app/api/getExpenses?id=${id}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
async function updateExpenses(credentials, id) {
  ...
  try {
    const data = await fetch(
      `https://us-central1-terp-338409.cloudfunctions.net/app/api/updateExpenses?id=${id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export default function EditExpense(props) {
  ...
  const [categories, setCategories] = useState(expensesTemp.category);
  const [expensesProjectsTemp, setExpensesProjectsTemp] = useState([]);
  const [expensesProjectName, setExpensesProjectName] = useState();

  // const people_x = [];
  // people.map((e) => people_x.push(e.name));

  const [updateExpensesHtmlValue, setUpdateExpensesHtmlValue] =
    useState("Save Changes");
  const [updateExpensesIcon, setUpdateExpensesIcon] = useState(faPlus);
  const [updateExpensesIconClassname, setUpdateExpensesIconClassname] =
    useState();
  ...
  useEffect(async () => {
    ...
    let mounted = true;
    const data = await getExpense(props.id);
    console.log(data);
    if (data.status == 403) {
      props.updateUser({});
      ...
      if (mounted) {
        ...
        setAuthorisedBy(data.data.authorisedBy);
  ...
  useEffect(async () => {
    setCategories(
      expensesTemp.category.filter((e) => {
  ...
  useEffect(async () => {
    console.log(expensesProjectsTemp);
    setExpensesProjectName(expensesProjectsTemp[0]?.name);
  }, [expensesProjectsTemp]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    document.getElementById("editExpenses-button").disabled = false;
    setUpdateExpensesHtmlValue("Processing please wait....");
    setUpdateExpensesIcon(faSpinner);
    setUpdateExpensesIconClassname("spinner");
    ...
    if (data.status == 403) {
      ...
      setUpdateExpensesIconClassname("");
      setUpdateExpensesIcon(faPlus);
      setUpdateExpensesHtmlValue("Save Changes");
      document.getElementById("editTask-button").disabled = false;
    } else if (data.status == 200) {
      ...
      document.getElementById("UpdateExpenses-form").reset();
      setUpdateExpensesIconClassname("");
      setUpdateExpensesIcon(faPlus);
      setUpdateExpensesHtmlValue("Save Changes");
      document.getElementById("editExpenses-button").disabled = false;
      ...
      <div className="xback-board txt-dark-bluex relative">
        <div className="modal-inner-edit-x">
          <div className="modal-edit-head-v2 relative">
            ...
            <h1 className=" p-2 pl-5 edit-modal-headr-x">Edit Expense</h1>
          </div>
          <form
            className="grid grid-cols-4 gap-4 px-6 py-6"
            id="UpdateExpenses-form"
          ...
          >
            ...
            <div className="col-span-4 text-left">
              ...
              >
                <option value="N/A">{authorisedBy}</option>
                {staffs.map((e) => {
                  return (
                    <option key={e.id} value={e.id}>
                      {e.name}
            ...
            <button
              ...
              id="editExpenses-button"
            >
              <FontAwesomeIcon
                icon={updateExpensesIcon}
                className={updateExpensesIconClassname}
              />{" "}
              {updateExpensesHtmlValue}{" "}
            </button>
          </form>
        </div>
      </div>
