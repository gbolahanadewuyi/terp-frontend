...
export default function DashHeaderDetails(props) {
  ...
  var status_cn;
  if(props?.status === 'Won' || props?.status === 'Completed' ){status_cn = 'ml-auto capitalize bg-green-100 text-green-500 rounded-lg text-s p-1 px-2'}
  else if(props?.status === 'Lost' || props?.status === 'Cancelled' ){status_cn = 'ml-auto capitalize bg-red-100 text-red-500 rounded-lg text-s p-1 px-2'}
  else if (props?.status === 'Pending' || props?.status === 'Submitted and Pending Approval' ){status_cn = 'ml-auto capitalize bg-amber-100 text-amber-500 rounded-lg text-s p-1 px-2'}
  return (
    <>
