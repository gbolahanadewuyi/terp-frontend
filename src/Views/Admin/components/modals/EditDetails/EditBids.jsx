...
import tempDb from "../../../../../tempDb/bids_temp";
import AdminRequestFunctions from "../../../adminRequestFunctions";
...
export default function EditBids(props) {
  ...
  const [predefinedBidStatuses, setPredefinedBidStatuses] = useState(tempDb.BidStatuses)
  ...
  useEffect(()=>{
    console.log(tempDb.BidStatuses)
    setPredefinedBidStatuses(
      tempDb?.BidStatuses?.filter((e)=>{
  ...
  useEffect( () => {
   async function getCompanies(){
    let mounted = true;
     try {
      const data = await AdminRequestFunctions.getCompanies();
      console.log(data);
      if (data.status === 403) {
  ...
  const handleSubmit = async (e) => {
    ...
    const data = await AdminRequestFunctions.updateBid(
      {
        tenderTitle,
        client,
