...
import {setDataMeeting} from '../../../../redux/actions/setData';

const MeetingCards = ({ data, rowsPerPage, handleShowDetails }) => {
  const [page, setPage] = useState(1);
  const { slice, range } = useTable(data, page, rowsPerPage);

  // const currentData = useSelector(state => state.currentDataMeetings);
              ...
              return(
                <div className='bg-whitex rounded txt-darkblue2 shadow-lg' key={e.id}>
                  ...
                  <div className='w-full bborder border-t-2 flex px-2 text-sm'>
                    ...
                    <button className='bg-bluex text-white px-2 py-1 rounded my-1 ml-auto' onClick={()=>{
                      dispatch(setDataMeeting(e));
                      handleShowDetails();
                    }}
                    >View Details</button>
                  </div>
