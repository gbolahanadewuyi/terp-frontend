...
export default function ProjectDetails(props) {
  ...
  const getProjectDetail = async (id) => {
    const data = await AdminRequestFunctions.getProjectDetails(id);
    console.log(data);
    if (data.status === 403) {
  ...
  useEffect(() => {
    async function fetchProjectDetails() {
      ...
      try {
        const data = await AdminRequestFunctions.getProjectDetails(props.id);
        console.log(data);
        if (data.status === 403) {
          toast.error(`${data.message}`, {
            icon: "😞",
  ...
  useEffect(() => {
    async function fetchProjectExpenses() {
      ...
      try {
        const data = await AdminRequestFunctions.getProjectExpensis(props.id);
        console.log(data);
        if (data.status === 403) {
          toast.error(`${data.message}`, {
            icon: "😞",
                ...
                <div className="side-content col-span-7 md:col-span-4 lg:col-span-2">
                  ...
                  <div className="mt-8">
                    ...
                    <ul>
                      ...
                      <li className="flex mt-2">
                        <DocumentCopyIcon classx="fill-current txt-bluex mr-3" />
                        {details?.certificateOfCompletion !== "" ? (
                          <a
